...
- Callback URL canonical cho social auth:
  - shared: `/ul/auth/callback?provider=instagram|tiktok|facebook`
  - instagram: `/ul/auth/instagram/callback`
  - tiktok: `/ul/auth/tiktok/callback`
  - facebook: `/ul/auth/facebook/callback`
- TikTok app config:
  - phải dùng callback path riêng `/ul/auth/tiktok/callback`
  - không dùng shared callback có query string vì TikTok reject URL chứa `?`
...
| ------ | ------------------------ | ------ | ------------- | ------------------------- | --------------------------------------------------------------------- |
| `POST` | `/auth/register`         | No     | Public        | `body: RegisterEmailDto`  | Tạo tài khoản với role mặc định là `user`, auto tạo free subscription |
| `POST` | `/auth/register-admin`   | Bearer | `admin`       | `body: RegisterAdminDto`  | Tạo account role admin/staff/user bởi admin hiện tại                  |
...
| `GET`  | `/auth/social-callbacks` | No     | Public        | none                      | Trả metadata callback URL và registration URL cho social auth         |
...
`UpdateUserAutofillDto`

```json
{
  "old_address": {
    "provider-specific-field": "string?"
  }
}
```
...
- Mỗi request chỉ được gửi `old_address` hoặc `new_address`.
- Nếu cần cập nhật cả hai, FE phải gọi API 2 lần.

`GET /users/me` response excerpt
...
```json
{
  "messageTemplate": {
    "order": {
      "template": [{ "content": "Có chứa {{variable}}", "isActive": true }],
      "image": "string?"
    },
    "comment": [{ "content": "Có chứa {{variable}}", "isActive": true }],
    "backup": [{ "content": "Có chứa {{variable}}", "isActive": true }],
    "error": [{ "content": "Có chứa {{variable}}", "isActive": true }],
    "newCustomer": [{ "content": "Có chứa {{variable}}", "isActive": true }]
  }
}
```
...
| `PATCH`  | `/users/me/autofill/:provider`                                                          | Bearer | Authenticated           | `param: provider`, `body: UpdateUserAutofillDto` | Cập nhật autofill địa chỉ của user         |
...
- `autofill` hiện nằm ở cấp `user`, không nằm trong `shops[]`.
- FE nên dùng `GET /users/me` để đọc `data.autofill`.
- `provider` nên dùng một trong các giá trị: `ghn`, `ghtk`, `jt-express`.
- Với J&T: path param dùng `jt-express`, còn key response là `jtExpress`.
- `PATCH /users/me/autofill/:provider` không cho phép gửi đồng thời `old_address` và `new_address`.
...
```json
{
  ...
  "quantity": "number >= 1?",
  "totalPrice": "number >= 0?",
  "status": "pending|delivering|success|faild|cancel?",
  "actionType": "NORMAL|BACKUP|CONFIRMED_ERROR?",
  "isNewCustomer": "boolean?"
}
```
...
- `pending`: trạng thái mặc định khi order mới được tạo.
- `delivering`: đang giao hàng.
- `success`: hoàn tất thành công.
- `faild`: giao/cập nhật thất bại. FE cần dùng đúng literal `faild` theo API hiện tại.
- `cancel`: đơn đã hủy.
...
| `PATCH`  | `/orders/:id/status`              | Bearer | `admin,user,staff` | `body: UpdateOrderStatusDto`               | Cập nhật riêng trạng thái order              |
...
| Provider  | Callback URL cần đăng ký trên provider                             | Shared callback alternative                                                 | Notes                                                                                                                              |
| --------- | ------------------------------------------------------------------ | --------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- |
| Instagram | `https://livetracker-ulz2.onrender.com/ul/auth/instagram/callback` | `https://livetracker-ulz2.onrender.com/ul/auth/callback?provider=instagram` | Backend có xử lý OAuth completion thật qua `InstagramOAuthService`                                                                 |
| TikTok    | `https://livetracker-ulz2.onrender.com/ul/auth/tiktok/callback`    | Không khuyến nghị cho TikTok app config                                     | TikTok yêu cầu callback URL thuần `https://...` và reject query string; backend chưa có token exchange service trong repo hiện tại |
| Facebook  | `https://livetracker-ulz2.onrender.com/ul/auth/facebook/callback`  | `https://livetracker-ulz2.onrender.com/ul/auth/callback?provider=facebook`  | Hiện route chỉ chuẩn hóa callback + Universal Link fallback, chưa có token exchange service trong backend                          |

- Nếu cấu hình TikTok Developer/App dashboard:
  - dùng đúng `https://livetracker-ulz2.onrender.com/ul/auth/tiktok/callback`
  - không thêm query params hay fragment

- Nếu login Instagram bắt đầu từ web `app.livetracker.vn`:
  - đặt `redirect_uri` về callback URL Instagram canonical:
    - `https://livetracker-ulz2.onrender.com/ul/auth/instagram/callback`
  - hoặc dùng shared callback:
    - `https://livetracker-ulz2.onrender.com/ul/auth/callback?provider=instagram`
...
- Nếu login bắt đầu từ app mobile:
  - dùng cùng `redirect_uri` cho đúng provider
  - gắn `state=app` hoặc payload tương đương
- Kết quả:
  - `web` → callback được chuyển sang `https://app.livetracker.vn/<same-path>?...`
  - `app` → Universal Link vẫn mở app như cũ
- Legacy Instagram:
  - `/ul` và `/ul/instagram-auth-callback` vẫn được support để không làm vỡ cấu hình cũ
...
- `delivery`: các endpoint `create-order` thường dùng `bizContent` bọc payload gốc của provider
...
Module `delivery` hỗ trợ tích hợp với các đơn vị vận chuyển (J&T Express, GHN, GHTK, ViettelPost).

### Tài liệu chi tiết từng Provider:

- **J&T Express**: [JT_EXPRESS_CONFIG.md](./src/modules/delivery/JT_EXPRESS_CONFIG.md)
- **GHN (Giao Hàng Nhanh)**: [GHN_CONFIG.md](./GHN_CONFIG.md)
- **GHTK (Giao Hàng Tiết Kiệm)**: [GHTK_CONFIG.md](./GHTK_CONFIG.md)
- **ViettelPost**: [VIETTEL_POST_CONFIG.md](./src/modules/delivery/VIETTEL_POST_CONFIG.md)

### Hướng dẫn tích hợp chung cho FE:

Xem chi tiết tại: [FE_DELIVERY_API_GUIDE.md](./docs/FE_DELIVERY_API_GUIDE.md)

### Các API chính:

| Nhóm | Path | Mô tả |
|---|---|---|
| Catalog | `GET /delivery/providers` | Lấy danh sách provider và trạng thái cấu hình |
| Config | `GET /delivery/providers/:provider/config` | Lấy cấu hình (masked) |
| Config | `PUT /delivery/providers/:provider/config` | Cập nhật cấu hình |
| History | `GET /delivery/orders` | Danh sách lịch sử vận đơn, hỗ trợ filter theo `orderId`, `provider`, `providers` |
| Operations | `POST /delivery/providers/:provider/create-order` | Tạo vận đơn |
| Operations | `POST /delivery/providers/:provider/cancel-order` | Hủy vận đơn |
| Operations | `POST /delivery/providers/:provider/order-detail` | Lấy chi tiết/tracking |
| Operations | `POST /delivery/providers/:provider/calculate-fees` | Tính phí vận chuyển |
